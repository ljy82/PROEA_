...
class Train_index_generator(object):
    ...
    def __next__(self):
        if self.iter_count < self.batch_num:
            ...
            return pos_f_ids, neg_f_ids, pos_pairs
        else:
            self.iter_count = 0
            self.train_pair_indexs, self.batch_num = self.train_pair_index_gene()
            raise StopIteration()
...
def one_step_train(Model, Optimizer, Criterion, icl_criterion, ial_criterion, Train_gene, f_emb, cuda_num):
    epoch_loss = 0
    # print("emb:",f_emb)
    # print(Train_gene)
    for pos_f_ids, neg_f_ids, pos_pairs in Train_gene:
        ...
        icl_loss = icl_criterion(f_emb, pos_pairs)
        ial_loss = ial_criterion(f_emb[:, :42], f_emb, pos_pairs) + ial_criterion(f_emb[:, 42:84], f_emb, pos_pairs) + ial_criterion(f_emb[:, 84:85], f_emb, pos_pairs) + ial_criterion(f_emb[:, 85:86], f_emb, pos_pairs) 

        # 模态内的对比学习损失
        nei_loss = icl_criterion(f_emb[:, :42], pos_pairs)
        att_loss = icl_criterion(f_emb[:, 42:84], pos_pairs)
        des_loss = icl_criterion(f_emb[:, 84:85], pos_pairs)
        onto_loss = icl_criterion(f_emb[:, 85:86], pos_pairs)

        # 总损失
        total_loss = batch_loss + icl_loss + ial_loss + nei_loss + att_loss + des_loss + onto_loss
...
def train(Model, Optimizer, Criterion, icl_criterion, ial_criterion,  Train_gene, f_emb_list, test_candidate, test_ill,
          entpair2f_idx, epoch_num, eval_num, cuda_num, test_topk):
    feature_emb = torch.FloatTensor(f_emb_list).cuda(cuda_num)
    print("start training interaction model!")
    for epoch in range(epoch_num):
        start_time = time.time()
        epoch_loss = one_step_train(Model, Optimizer, Criterion, icl_criterion, ial_criterion, Train_gene, feature_emb, cuda_num)
